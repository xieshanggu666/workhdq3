/** 一键流水线选择弹窗：预设产线卡片（科技门控置灰） */
import Modal from './Modal.jsx';
import { ui } from '../state/ui';
import { FG } from '../engine';

export default function PipelinesModal() {
  const close = () => ui.set({ modal: null });
  const game = FG.game;
  if (game.state !== 'playing') { close(); return null; }
  ...
  return (
    <Modal onClose={close}>
      ...
      <div style={{ color: 'var(--text-dim)', fontSize: 11, lineHeight: 1.7, marginBottom: 10 }}>
        ...
        施工计划自动从<b>箱子/地面物料堆</b>预留建材，缺料挂起等待，建成即接入生产调度。
      </div>
      <div className="pl-grid">
        {presets.map(p => {
          ...
          const costStr = Object.keys(cost).map(k => `${FG.Items.byId(k).name}×${cost[k]}`).join(' · ');
          return (
            <div key={p.id}
